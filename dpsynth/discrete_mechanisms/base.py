...
class DiscreteMechanismConfig(api.MechanismConfig):
  ...
  def configure(self, *, zcdp_rho, delta=0, max_records_per_user=1):
    ...
    if self.one_way_budget_fraction <= 0:
      one_way_rho = None
    else:
...
class DiscreteMechanism(api.CalibratedMechanism):
  ...
  def __call__(
      ...
      *,
      # By default we set one_way_budget_fraction = 1/3 in the config.
      # If initial_measurements is provided here, then we should be able to save
      # that budget and use it in the subsequence select + measure steps.
      # Currently, this budget is just wasted, which is a code smell and
      # utility issue. This will be resolved in a near-term future refactoring.
